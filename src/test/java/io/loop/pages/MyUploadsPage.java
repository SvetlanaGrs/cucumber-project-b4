...
public class MyUploadsPage {
    ...
    public void insertField(String field, String input)  {
        switch (field.toLowerCase().trim()){
            case "upload file":
                // BrowserUtils.waitForVisibility(uploadFile,10).sendKeys(input,Keys.ENTER);
               // Actions actions = new Actions(Driver.getDriver());
               // actions.moveToLocation(-100,-100).click().sendKeys(input).perform();
               // actions.sendKeys(input,Keys.ENTER).perform();
               // BrowserUtils.waitForVisibility(uploadFile,10).sendKeys(input);
                break;
